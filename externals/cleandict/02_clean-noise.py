...
def load_env_variables(key_name="GOOGLE_API_KEY_1"):
    ...
    # 核心邏輯：根據輸入的參數決定要用哪一個 Key，如果找不到則默認用 GOOGLE_API_KEY_1
    target_key = os.environ.get(key_name)
    if not target_key:
        # 如果找不到指定的 Key，嘗試用 GOOGLE_API_KEY_1
        key_name = "GOOGLE_API_KEY_1"
        target_key = os.environ.get(key_name)
        print(f"⚠️ 找不到指定的 Key，改用默認: {key_name}")

    if target_key:
        os.environ["GEMINI_API_KEY"] = target_key
        # 清除其他的 API Key 以避免優先級衝突
        for i in range(1, 6):
            env_key = f"GOOGLE_API_KEY_{i}"
            if env_key in os.environ and env_key != key_name:
...
def process_dict():
    ...
    # 1. 根據 --part 自動選擇對應的 API Key，如果找不到則默認用 GOOGLE_API_KEY_1
    key_name = f"GOOGLE_API_KEY_{args.part}"
